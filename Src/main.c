...
extern volatile uint8_t  timeoutFlgGen; // Timeout Flag for the General timeout (PPM, PWM)
...
int main(void) {
  ...
  while(1) {
    ...
    calcAvgSpeed();                       // Calculate average measured speed: speedAvg, speedAvgAbs
      ...
      #if defined(TANK_STEERING) && !defined(VARIANT_HOVERCAR) 
    ...
    if (TEMP_POWEROFF_ENABLE && board_temp_deg_c >= TEMP_POWEROFF && speedAvgAbs < 20){  // poweroff before mainboard burns OR low bat 3
      ...
      beepCount(3, 24, 1);
    } else if (timeoutFlgGen) {                                                                       // 4 beeps (low pitch): General timeout (PPM, PWM)
      beepCount(4, 24, 1);
